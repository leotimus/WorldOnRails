...
class ImageAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.steers = torch.tensor(np.linspace(-self.max_steers, self.max_steers, self.num_steers)).float().to(
            self.device)
    ...
    def create_masking(self, wide_rgb, center, size, radius):
        ...
        im = occlude(wide_rgb.squeeze(), mask).reshape(240,
                                                       480)  # (3, 480, 240) --> ValueError: operands could not be broadcast together with shapes (240,480,3) (480,240)
        return im
    ...
    def sensors(self):
        sensors = [
            ...
            {'type': 'sensor.stitch_camera.rgb', 'x': self.camera_x, 'y': 0, 'z': self.camera_z, 'roll': 0.0,
             'pitch': 0.0, 'yaw': 0.0,
             'width': 160, 'height': 240, 'fov': 60, 'id': f'Wide_RGB'},
            {'type': 'sensor.camera.rgb', 'x': self.camera_x, 'y': 0, 'z': self.camera_z, 'roll': 0.0, 'pitch': 0.0,
             'yaw': 0.0,
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.all_speeds:
            ...
            self.saliency_inputs.append(ias)
            # Interpolate logits
            steer_logit = self._lerp(steer_logits, spd)
            throt_logit = self._lerp(throt_logits, spd)
            brake_logit = self._lerp(brake_logits, spd)
        else:
            steer_logit, throt_logit, brake_logit = self.image_model.policy(_wide_rgb, _narr_rgb, cmd_value,
                                                                            spd=torch.tensor([spd]).float().to(
                                                                                self.device))
