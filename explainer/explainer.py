    ...
    def score_frame(self, saliency_info, density=10, radius=5):
        ...
        for i in range(0, 120, density):
            for j in range(0, 240, density):
                ...
                scores_throttle[x, y] = current_score_throttle #0-24/0-48, get Difference value in place 0/0 --> 0.5198/ brake_Logits / brake_logits.pow(2).sum().mul(.5)
        ...
        erased_gray_score_throttle = np.where(scores_denoised_throttle <= movsd_throttle , 0,
        ...
        erased_gray_score_brake = np.where(scores_denoised_brake <= movsd_brake , 0,
        ...
        erased_gray_score_steer = np.where(scores_denoised_steer <= movsd_steer, 0,
